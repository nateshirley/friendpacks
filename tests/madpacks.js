...
const { Keypair, PublicKey, SystemProgram, SYSVAR_RENT_PUBKEY, Transaction } =
  anchor.web3;
const SPL = require("@solana/spl-token");
const { TOKEN_PROGRAM_ID, Token, MintLayout } = SPL;
const {
  getPackMintKeysForWallet,
  getMetadataAddress,
  getAssociatedTokenAccountAddress,
  createAssociatedTokenAccountInstruction,
  TOKEN_METADATA_PROGRAM_ID,
  getPackMintAccountsForWallet,
  fetchAllPackMintAccounts,
  getMembersForPackMint,
  isWalletPackMember,
  isPackEligibleForNewMembers,
  buildConnectedMembersDict,
...
describe("madpacks", () => {
  ...
  let TokenMint = null;
  //cli wallet
  let myWalletPubkey = new PublicKey(
    "5J8jLVz5YY5uc9sJuWtx42VVMUanLGYWoPXMRu7GsNEJ"
  );
  ...
  it("config", async () => {
    ...
    payerTokenAccount = await getAssociatedTokenAccountAddress(
      payer.publicKey,
      mint.publicKey
    );
    ...
    let [_metadataAddress, _metadataBump] = await getMetadataAddress(
      mint.publicKey
    );
  ...
  it("join the again and again", async () => {
    let secondTokenAccount = await getAssociatedTokenAccountAddress(
      secondMember.publicKey,
      mint.publicKey
    );
  ...
  */
  ...
  it('get token accounts owned by my test owner address', async () => {
    ...
    let accountResponses = await fetchAllPackMintAccounts(provider.connection);
    console.log("number of pack mints in total, ", accountResponses.length);
    console.log(accountResponses)
    accountResponses.forEach((accountResponse) => {
      let creator = accountResponse.account.data.slice(326, 358);
      console.log(creator);
      console.log(new PublicKey(creator).toBase58());
      let isVerified = accountResponse.account.data.slice(358, 359);
